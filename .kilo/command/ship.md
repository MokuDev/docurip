...
Spawn three subagents concurrently. **Issue all three calls in a single turn so they execute in parallel** — sequential calls defeat the purpose of this command.
...
If parallel agent execution is not available, run each persona sequentially and treat their outputs as if returned in parallel — the merge phase still works.
