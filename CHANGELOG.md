...
- **Scheduled / recurring crawls**: crawl a site automatically on a daily, weekly, or monthly cadence at a fixed UTC time (with weekday for weekly and day-of-month, clamped to 1–28, for monthly). A new `scheduler` module persists schedules through the generic `JsonStore<Schedule>` and runs an in-process ticker that wakes once a minute; its first tick fires immediately on launch, so a schedule missed while the app was closed runs at startup (**startup catch-up**). This in-process approach was chosen over an OS-level scheduler to fit the offline-first, single-user scope. New `list_schedules` / `save_schedule` / `delete_schedule` / `toggle_schedule` commands (with server-side `nextRun` computation and `validate_crawl_input` reused at save time and before each run) back a new **Scheduled** view for creating, editing, pausing, and deleting schedules; next/last run are shown in local time. A schedule takes one URL per line: a single URL runs a plain crawl, two or more run through the batch queue (with a per-schedule Continue/Stop override that falls back to the Settings default), and a saved template can be picked to supply the crawl settings — the config is copied in as a snapshot, so later template edits leave existing schedules alone.

- **Configurable Live Console history**: a new `liveConsoleMaxEvents` setting (Settings → General → Live Console, default 1000, range 100–10,000) controls how many log lines the console retains before dropping the oldest. Takes effect the next time the console is opened.
- **Keyboard shortcut for the Scheduled view**: default Ctrl/Cmd+S, registered in the central shortcut registry so it also appears in Settings → Keyboard Shortcuts with rebind + conflict detection, and its hint shows on the sidebar's Scheduled item.

### Changed
- `validate_crawl_input` is now `pub(crate)` so the scheduler can revalidate a stored schedule's URL/config before spawning.

### Fixed
- **Live Console froze after 500 events**: the console tracked which events it had rendered by their absolute index into the shared event buffer, but that buffer is a sliding window capped at 500 — once it filled, `startIdx >= events.length` held on every subsequent event and the console stopped updating entirely. Events now carry a stable monotonic `seq` assigned as they arrive, and the console drains by "seq greater than last shown", so it keeps updating no matter how many events stream through. The shared buffer cap was also raised (500 → 2000) and decoupled from the console's now user-configurable retained-history limit.
...
- **Search highlighting in Markdown preview**: matches in the preview pane are now wrapped in `<mark>` tags with per-match indices; a floating navigator in the preview shows `current / total` and lets you jump between matches with prev/next arrows, scrolling the active match into view. Fixes a pre-existing bug where highlighting used a raw string `.replace` over already-rendered HTML — searching for a common word like `text` would collide with Tailwind class names such as `text-ghost` and corrupt the markup. Highlighting now walks text nodes via `TreeWalker`, skipping tag attributes, `<mark>` (idempotent re-runs), and `<script>` / `<style>`.
- **Annotations for crawled pages**: an inline notes panel under the Markdown preview lets you attach free-text notes to any page. Notes autosave on a 700 ms debounce (and force-flush on blur so switching pages never loses a keystroke), with a Saving/Saved/Unsaved indicator. Pages that carry a note get a small note-pencil marker in the ResultTree. Notes are stored on the job as `annotations: HashMap<String, String>` (URL → text), serde-defaulted so old on-disk jobs load unchanged. New `set_annotation` Tauri command persists updates (empty text deletes).
...
### Security
- **Redirect-based SSRF bypass in the sitemap fetcher**: `is_private_target` was only checked on the initial URL; reqwest's default redirect policy followed up to 10 hops, so a hostile server could 302 to `http://127.0.0.1/…` and land the request there. The sitemap client now installs a custom `reqwest::redirect::Policy` that runs `is_private_target` on every hop and refuses to follow.
- **SSRF bypass for URLs 2..N of a batch crawl**: `start_batch` only ran `validate_crawl_input` on `urls.first()`; the follow-up loop only checked the scheme, so a private-address host on the 2nd+ URL bypassed the SSRF gate (since `spawn_crawl` trusts its caller). Every URL now goes through `validate_crawl_input`.
- **Server-side batch size cap**: `start_batch` only rejected an empty list; the 500-URL cap was frontend-only, so a direct `invoke("start_batch", …)` could push arbitrarily large payloads. Added `MAX_BATCH_URLS = 500` and enforced it at the command boundary.
- **Sitemap response body was buffered before the size check**: `resp.bytes()` loaded the whole payload before comparing to `MAX_BODY_BYTES`, and `Content-Length` was the only pre-check — trivially spoofable. Replaced with a chunk-by-chunk stream that rejects as soon as the running total exceeds the cap, and applied the same streaming path to `robots.txt` with a tighter 1 MB cap (`MAX_ROBOTS_BYTES`).

### Fixed
- **`JsonStore::init` aborted on per-file IO errors**: the `?` on `std::fs::read_to_string` meant a single unreadable file (Windows file-lock, transient permission error) aborted startup even though the docstring promised tolerance. Per-file IO errors now `continue` like malformed JSON already does.
- **Batch runner could hang on a stuck child job**: `wait_for_terminal` polled forever with no upper bound. Added a `CHILD_TIMEOUT` (30 min) after which the runner forwards a stop signal, waits `STOP_GRACE` seconds, and abandons the child as failed so the rest of the batch can proceed.
- **Batch reported only the first child failure**: `error_msg.get_or_insert(...)` kept the first error and discarded the rest. A 100-URL Continue-mode batch where 30 fail now shows all 30 messages on the batch record, prefixed with a count.
- **LiveConsole couldn't be closed while a crawl was running**: the auto-open effect fired on every render where `activeCount > 0 && !liveConsoleOpen`, so closing it immediately re-opened it. Now edge-triggered on a 0→N transition via a `useRef`, so the user can close it and it stays closed.
- **Sitemap fetch had no cancellation hook**: a user closing the picker mid-fetch left the recursion running to completion. Added `fetch_sitemap_with_stop` accepting an `AtomicBool` handle; the queue loop checks it between fetches and returns a truncated result.
- **`crawl-event` payloads emitted snake_case field names**: `#[serde(rename_all = "camelCase")]` on the `CrawlEvent` enum only renamed variant names for the `tag` field; struct fields inside variants kept `job_id` / `batch_id` etc. The frontend has always read `event.jobId`, so it was `undefined` at runtime. The whole event-driven surface (LiveConsole auto-open, "JOBS N" top-bar counter, notification deduplication, auto-export gating) "worked" only because `Set.add(undefined)` still bumps the size to 1 — a coincidence that broke as soon as any code path guarded against a falsy `event.jobId`. Added `rename_all_fields = "camelCase"` to the enum so payloads now actually match what the frontend expects, and covered every variant with serialization tests to prevent regressions.
- **Crawls on Windows silently wrote to the drive root when the user hadn't chosen an output directory**: the frontend uses an empty `outputDir` as a "use default" marker, and once Settings was opened+saved once, that empty string overwrote the store. The orchestrator then produced `format!("{}/{}", "", host)` → `/docs.example.com`, which Windows interprets as an absolute path on the current drive — so crawl output ended up in `C:\docs.example.com\` instead of `C:\Users\<name>\.docurip\docs.example.com\`. `get_settings` now normalizes an empty stored `output_dir` back to the resolved default before returning, so every consumer (orchestrator, Settings display, History's "Open folder") sees an absolute path.
- **LiveConsole showed every event twice in dev**: React.StrictMode double-mounts the `crawl-event` listener effect. `listen()` is async, so the first cleanup ran while the `unlisten` handle was still `undefined` — both listen attempts survived and every event landed twice. Added a `cancelled` flag so a late-resolving handle is unsubscribed immediately if teardown has already fired.
- **LiveConsole didn't auto-open on batch activity**: the auto-open effect keyed off `activeJobIds.size`, which only populates from `jobStatusChanged`. A running batch that hadn't yet spawned its first child left the set empty, so the console stayed closed. `useCrawlEvents` now also tracks an `activeBatchIds` set fed by `batchStatusChanged`, and the console opens whenever either set is non-empty. `LiveConsole` also formats `batchProgress` / `batchStatusChanged` events natively instead of rendering them as "Unknown event".
