...
Specialist personas that play a single role with a single perspective. Each persona is a Markdown file consumed as a system prompt by your harness (Kilo Code, Cursor, Copilot, etc.).
...
2. **Personas do not invoke other personas.** Composition is the job of slash commands or the user. In Kilo Code, spawned agents cannot recursively spawn further agents — the rule is enforced by the platform.
